 ...
 * â€œLinuxâ€™s hotplugging is very inefficient.â€
 *
 ...
 * some problems with the â€œAlways onâ€ approach. On situations such as video
 ...
 * the next poll determines 1 core isnâ€™t enough, it fires up all CPU cores
 * (instead of selective CPU cores; which is the traditional intelli_plugâ€™s
 * method).
 * Lazyplug also takes touch-screen input events to fire up CPU cores to
 * minimize noticeable performance degradation.
 * Thereâ€™s also a â€œlazy modeâ€ for *not* aggressively turning on CPU cores
 * on scenario such as video playback. For example, if you hook up
 * lazyplug_enter_lazy() to the video session open function, Lazyplug wonâ€™t
