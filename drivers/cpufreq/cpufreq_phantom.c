 ...
 *
 *
 * Added Screen off Max frequency. Gobinda Joy/gj86 (gobinda.joy@gmail.com)
 *
 */
...
#define DEFAULT_SCREEN_OFF_MAX_FREQ 652800
...
static DEFINE_PER_CPU(struct cpufreq_phantom_cpuinfo, cpuinfo);
...
static unsigned int screen_off_max_frequency = DEFAULT_SCREEN_OFF_MAX_FREQ;
...
static void cpufreq_phantom_timer_resched(
	struct cpufreq_phantom_cpuinfo *pcpu)
{
 ...
 * function.
 */
static void cpufreq_phantom_timer_start(int cpu, int time_override)
{
...
static unsigned int choose_freq(
	struct cpufreq_phantom_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_phantom_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_phantom_timer_resched(pcpu);
...
static void cpufreq_phantom_idle_start(void)
{
	struct cpufreq_phantom_cpuinfo *pcpu =
...
static void cpufreq_phantom_idle_end(void)
{
	struct cpufreq_phantom_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_phantom_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_phantom_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_phantom_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
...
static void cpufreq_phantom_boost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
...
static int cpufreq_phantom_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_phantom_notifier,
};
...
static ssize_t show_screen_off_max_frequency(struct kobject *kobj,
				struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", screen_off_max_frequency);
}

static ssize_t store_screen_off_max_frequency(struct kobject *kobj,
				struct attribute *attr, const char *buf,
				size_t count)
{
	int ret;
...
static struct attribute *phantom_attributes[] = {
	...
	&screen_off_max_frequency_attr.attr,
	NULL,
};
...
static int cpufreq_phantom_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_phantom_idle_start();
		break;
	case IDLE_END:
		cpufreq_phantom_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_phantom_idle_nb = {
	.notifier_call = cpufreq_phantom_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (cpu_online(j))
				cpufreq_phantom_timer_start(j, 0);
		...
		idle_notifier_register(&cpufreq_phantom_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_phantom_idle_nb);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (policy->min >= pcpu->target_freq) {
				...
				cpufreq_phantom_timer_start(j, expire_time);
			}
...
static void phantom_suspend(struct power_suspend *handler)
{
	power_suspended = true;
	return;
}

static void phantom_resume(struct power_suspend *handler)
{
	power_suspended = false;
	return;
}

static struct power_suspend phantom_power = {
...
static void cpufreq_phantom_nop_timer(unsigned long data)
{
}

static int __init cpufreq_phantom_init(void)
{
	unsigned int i;
	struct cpufreq_phantom_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_phantom_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_phantom_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_phantom_speedchange_task, NULL,
			       "cfphantom");
...
fs_initcall(cpufreq_phantom_init);
#else
module_init(cpufreq_phantom_init);
#endif
...
module_exit(cpufreq_phantom_exit);

MODULE_AUTHOR("Ghost");
MODULE_DESCRIPTION("'cpufreq_phantom' - A cpufreq governor for "
