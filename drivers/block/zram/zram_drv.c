...
static void zram_page_end_io(struct bio *bio, int err)
{
...
static void zram_sync_read(struct work_struct *work)
{
	...
	read_from_bdev_async(zram, &bvec, entry);
}
...
static int read_from_bdev(struct zram *zram, struct bio_vec *bvec,
			unsigned long entry, bool sync)
{
	if (sync)
		return read_from_bdev_sync(zram, bvec, entry);
	else
		return read_from_bdev_async(zram, bvec, entry);
}

static int write_to_bdev(struct zram *zram, struct bio_vec *bvec,
					u32 index, unsigned long *pentry)
{
	struct bio *bio;
	unsigned long entry;
				...
				u32 index, int offset)
{
	...
	ret = __zram_bvec_read(zram, page, index, is_partial_io(bvec));
	if (unlikely(ret))
		goto out;
...
compress_again:
	...
	if (unlikely(comp_len > max_zpage_size)) {
		...
		if (zram_wb_enabled(zram) && allow_wb) {
			zcomp_stream_put(zram->comp);
			ret = write_to_bdev(zram, bvec, index, &element);
			if (!ret) {
				flags = ZRAM_WB;
				ret = 1;
				goto out;
				...
				u32 index, int offset)
{
	...
	if (is_partial_io(bvec)) {
		...
		ret = __zram_bvec_read(zram, page, index, true);
		if (ret)
			goto out;
...
static DEVICE_ATTR(mem_limit, S_IRUGO | S_IWUSR, NULL,
		mem_limit_store);
static DEVICE_ATTR(mem_used_max, S_IRUGO | S_IWUSR, NULL,
		mem_used_max_store);
