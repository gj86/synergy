 ...
 */
static void guarantee_online_cpus(struct cpuset *cs,
...
{
	while (!nodes_intersects(cs->effective_mems, node_states[N_HIGH_MEMORY]))
		cs = parent_cs(cs);
	nodes_and(*pmask, cs->effective_mems, node_states[N_HIGH_MEMORY]);
}
...
static int validate_change(struct cpuset *cur, const struct cpuset *trial)
{
	struct cgroup *cgrp;
	struct cpuset *c, *par;
	int ret;
				       ...
				       size_t nbytes, loff_t *ppos)
{
	...
	switch (type) {
	case FILE_CPULIST:
		s += cpuset_sprintf_cpulist(s, cs->cpus_requested);
		break;
	case FILE_MEMLIST:
...
static int cpuset_populate(struct cgroup_subsys *ss, struct cgroup *cgrp)
{
	int err;

	err = cgroup_add_files(cgrp, ss, files, ARRAY_SIZE(files));
	if (err)
		return err;
	/* memory_pressure_enabled is in root cpuset only */
	if (!cgrp->parent)
		err = cgroup_add_file(cgrp, ss,
...
static struct cgroup_subsys_state *cpuset_create(struct cgroup *cgrp)
{
	struct cgroup *parent_cg = cgrp->parent;
...
static void cpuset_destroy(struct cgroup *cgrp)
{
	struct cpuset *cs = cgroup_cs(cgrp);
